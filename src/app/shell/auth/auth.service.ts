...
export class AuthService {
  constructor(@Inject(HttpClient) private http: HttpClient, private router: Router, private cryptSvc: CryptService) {}
  login(auth: Auth) {
    return this.http
  ...
  public forgot(email: string): Observable<boolean> {
    return this.http.get<DataResponse>('auth/' + email + '/forgot/').pipe(
      map((res: DataResponse) => {
        if (res.code === 1) {
          return true;
        }
  ...
  public reset(reset: any): Observable<boolean> {
    return this.http.post<DataResponse>('auth/reset/', reset).pipe(
      map((res: DataResponse) => {
        if (res.code === 1) {
          return true;
        }
