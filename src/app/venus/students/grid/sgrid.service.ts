...
@Injectable()
export class StudentsGridService {
  ...
  constructor(private store: SimpleStoreManagerService, private categoryRepSrv: StudentsRepoService) {
    this.pageGridConfig = {
      itemMap: {
        heading: 'name',
        description: '',
  ...
  onSelect(event: any, category: any) {
    this.store.add('studentsNavigatingId', category.id, true);
    this.navigateToForm();
  }
  navigateToForm() {
    this.store.setIn('studentspageconfig', ['pageHeading'], 'Category Form');
    this.store.setIn('studentspageconfig', ['pageBodyUrl'], 'app-category-form');
  }
}